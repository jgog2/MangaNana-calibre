    ...
    def search(self, query, offset=0, limit=12, include_adult=False, preferred='en', availability_cache=None):
        ...
        while len(rows) < limit and not exhausted:
            ...
            for entry in fetched:
                ...
                rows.append({'score':self._score(query,display,full_title,preferred in (attrs.get('availableTranslatedLanguages') or [])), 'title':display,'full_title':full_title,'alternate_titles':alternate_titles,'structured_titles':titles,'author':author,'year':attrs.get('year'),'available_languages':[str(x) for x in (attrs.get('availableTranslatedLanguages') or []) if x],'adult':attrs.get('contentRating') in ('erotica','pornographic'),'id':manga_id,'cover_url':f'https://uploads.mangadex.org/covers/{manga_id}/{cover}' if manga_id and cover else '','badge':badge})
                if len(rows)>=limit: break
